...
function rotate(event) {
    if (bGame) {
        ...
        let dy = canvas.height + y_fault - event.y;
        if (dy >= 0 && dx >= 0) {
...
Gun = new Class({
    draw: function(ctx) {
        with(this) {
            ...
            ctx.moveTo(15 - 50, 4);
            ctx.lineTo(30 - 100, 0 - 130);2
            ctx.lineTo(70 - 100, 0 - 143);
            ctx.lineTo(85 - 50, 0);
            ctx.lineTo(15 - 50, 0);
            ctx.closePath();
            ctx.fill();
            ...
            ctx.lineTo(520, canvas.height - 30);
            ctx.lineTo(580, canvas.height - 30);
...
function get_hp() {
    str = '';
    i = 0;
    while (i++ < hp) str += 'Выделенная аргент энергия';
    return str;
}
//name
function check_name() {
    firstName = prompt('Твое имя, боец: ');
...
function end_game() {
    alert("*Неразборчивые помехи* ПРИЕМ!! ВСЕМ ПОКИНУТЬ БАЗУ!!! *Помехи* АААааААААаААААаАа");
    localStorage.setItem(name, score);
    change_status();
}

function rules(){
    alert("Прием... штаб ОАК вызывает оператора BFG-10000... У тебя ответственное задание. Вторжение Палача Рока и его последующее разрушение поверхности Марса привела к катастрофе и последующему появлению огромного числа демонов... Внимание, ни одна из этих тварей не должна попасть к корпусу... Повторяю НИ ОДНА! Вся надежда на тебя....");
}
